    ...
    if (userAuth?.role !== 'SuperAdmin') {
      if (userAuth?.role === 'DistrictAdmin') {
        if (userAuth.district)
          whereClause.district = { in: normalizeJurisdiction(userAuth.district) };
      } else if (userAuth?.role === 'DivisionAdmin') {
        if (userAuth.division)
          whereClause.division = { in: normalizeJurisdiction(userAuth.division) };
...
export const updateComplaint = async (req: Request, res: Response) => {
  try {
      ...
      if (userAuth?.role === 'LabAdmin') {
        ...
        if (!institutes.includes(existingComplaint.institute)) {
          return res.status(403).json({
            success: false,
            message: 'Unauthorized: this complaint does not belong to your lab',
          });
        }
      } else {
    ...
    let updatedImages = existingComplaint.complaintImages
      ? [...existingComplaint.complaintImages]
      : [];
    ...
    if (deletedImages) {
      ...
      try {
        imagesToDelete =
          typeof deletedImages === 'string' ? JSON.parse(deletedImages) : deletedImages;
